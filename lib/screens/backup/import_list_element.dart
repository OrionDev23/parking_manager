...
class ImportListElement extends StatefulWidget {
  ...
  const ImportListElement({super.key, this.onRefresh, required this.title, this.subTitle, this.onChecked, required this.checked, required this.refreshing});
...
class _ImportListElementState extends State<ImportListElement> {
  ...
  Widget build(BuildContext context) {
    return ListTile(
      ...
      trailing: widget.refreshing?
          const ProgressRing()
          :IconButton(icon: const Icon(FluentIcons.refresh),onPressed: widget.onRefresh,),
    );
  }
}
