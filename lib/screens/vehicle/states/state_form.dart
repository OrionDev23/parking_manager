...
class StateFormState extends State<StateForm> {
  ...
  Future<void> downloadReparation(String id)async{
    ...
    await ClientDatabase.database!.getDocument(
        ...
        documentId: id).then((value) {
      if(value.data.isNotEmpty){
        selectedReparation=value.convertTo((p0) => Reparation.fromJson(p0 as Map<String,dynamic>));
      }
    });
    if(mounted){
      setState(() {
                ...
                children: <Widget>[
                  ListTile(
                     ...
                     leading:const Text('vehicule').tr(),
                      title: Text(selectedVehicle?.matricule??widget.vehicle?.matricule??widget.etat?.vehicleMat??'/'),
                      onPressed: widget.vehicle!=null || loadingVehicle || widget.etat?.vehicle!=null ?null:() async{
                  ...
                  ListTile(
                    ...
                    leading: Row(
                      children: [
                        ...
                        bigSpace,
                        Text('chstates',style: placeStyle.copyWith(fontWeight: FontWeight.bold)).tr(),
                      ],
                    ),
                    title: ComboBox<int>(
                  ...
                  ListTile(
                      ...
                      leading: SizedBox(
                          width: 120.px,
                          child: Text('date',style: placeStyle.copyWith(fontWeight: FontWeight.bold)).tr()),
                      title: Row(
                          crossAxisAlignment: CrossAxisAlignment.end,
                          children: <Widget>[
                            Expanded(
                              ...
                              child: DatePicker(
                                ...
                                onChanged: (s){
                                  if(s!=date){
                                    date=DateTime(s.year,s.month,s.day,date.hour,date.minute,date.second);
                            ...
                            Expanded(
                                ...
                                child:  TimePicker(
                                  ...
                                  onChanged: (s){
                                    if(date!=s){
                                      date=DateTime(date.year,date.month,date.day,s.hour,s.minute,s.second);
                                      setState(() {
                                      });
                                    }
                                  },
                  ...
                  if(type==2)
                    ListTile(
                      ...
                      title: Text(selectedReparation?.numero!=null ||widget.etat?.ordreNum!=null ?NumberFormat('00000000').format(selectedReparation?.numero??widget.etat?.ordreNum):'/'),
                      onPressed: loadingReparation || widget.etat?.ordreID!=null ?null:() async{
                        selectedReparation=await showDialog<Reparation>(context: context,
                            barrierDismissible: true,
                            builder: (context){
                              return  ContentDialog(
                                constraints: BoxConstraints.tight(Size(
                                ...
                                content: Container(
                                    ...
                                    width: 60.w,
                                    height: 60.h,
                                    child: const ReparationTable(selectD: true,)
                                ),
                              );
                            }
                        );
                        setState(() {
                          valeur=selectedReparation?.getPrixTTC()??valeur;
                        });
                      },
                    ),
                  if(type==2)
                  ...
                  ListTile(
                    ...
                    title: TextBox(
                      controller: TextEditingController(text: valeur.toStringAsFixed(2)),
                      onChanged: (String value) {
                  ...
                  ListTile(
                      ...
                      title: Row(
                          ...
                          children: <Widget>[
                        Text('assignnow',style: placeStyle.copyWith(fontWeight: FontWeight.bold),).tr(),
                        bigSpace,
                        Align(
  ...
  void submit()async {
    if(selectedVehicle==null && widget.etat?.vehicle==null && widget.vehicle==null){
    ...
    Etat etat=Etat(
        id: documentID!,
        vehicle: selectedVehicle?.id??widget.etat?.vehicle??widget.vehicle?.id??'',
        vehicleMat: selectedVehicle?.matricule??widget.etat?.vehicleMat??widget.vehicle?.matricule??'',
        remarque: remarque,
        type:type,
