...
class ConducteurWebService extends ParcOtoWebService<Conducteur> {
  ...
  @override
  int Function(
          MapEntry<String, Conducteur> p1, MapEntry<String, Conducteur> p2)?
    ...
    switch (column) {
      ...
      case 0:
        return (d1, d2) =>
            coef *
            '${d1.value.name} ${d1.value.prenom}'
                .compareTo('${d2.value.name} ${d2.value.prenom}');
      //vehicle
      case 1:
        return (d1, d2) {
      ...
      case 4:
        return (d1, d2) {
          if (d1.value.dateNaissance == null ||
              d2.value.dateNaissance == null) {
            return 0;
          } else if (d2.value.dateNaissance == d1.value.dateNaissance) {
            return 0;
          } else {
            return coef *
                d1.value.dateNaissance!.compareTo(d2.value.dateNaissance!);
          }
        };
      case 5:
        return (d1, d2) =>
      ...
      case 6:
        return (d1, d2) => coef * d1.value.etat!.compareTo(d2.value.etat!);
    }
  ...
  List<String> getFilterQueries(Map<String, String> filters, int count,
      ...
      {int? index}) {
    return [
      if (archive) Query.equal('etat', 3),
      if (!archive) Query.notEqual('etat', 3),
